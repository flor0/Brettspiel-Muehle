        ...
        if hand_remaining_ai > 0:
            ...
            if not blocker_move:
                ...
                if not board[to[0]][to[1]] == 0:
                    ...
                    to = ai_minimax_alpha_beta.Morris(board, generate_board_muhlen(board), player_ai, hand_remaining[player_ai]+hand_remaining[player_human])
                    if not board[to[0]][to[1]] == 0:
    ...
    while not done:
        ...
        if turn:
            if hand_remaining[player_ai] > 0:   # Game phase 1, placing
                ...
                # Call AI
                ai_move_to, ai_move_from = generate_move(board, player_ai, hand_remaining[player_human], hand_remaining[player_ai], board_remaining[player_human], board_remaining[player_ai])
                if not move_is_legal(board, ai_move_to, player_ai, board_remaining[player_ai], hand_remaining[player_ai]):   # Illegal move
        ...
        elif board_remaining[2] < 3:  # If White wins
            print("White won!")
            done = True

        # Victory by unable to move
        if not canmoveatall(1, board):  # If White can't move, Black wins
            print("Black won!")
            done = True
