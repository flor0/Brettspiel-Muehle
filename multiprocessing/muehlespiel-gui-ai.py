    ...
    while phase1_remaining and not done:
        if not turn:
            ...
            try:
                ...
                pass
            try:
                ai_move_from = eval(ai_out_data[2])
            except:
                pass
            print("denken fertig")
            print("AI MOVE: "+str(ai_move))
            spielfeld[ai_move[0]][ai_move[1]] = 2
            # If a mill has been created by the AI
            if checkmuhle(ai_move[0], ai_move[1]) and spielfeld[ai_remove[0]][ai_remove[1]] == 1:
                removeman(ai_remove[0], ai_remove[1])
            turn = not turn
            phase1_remaining -= 1
            drawBoard()
            drawState()
